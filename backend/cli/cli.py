...
project_opt = typer.Option(
    ...
    help="Path to the project directory (default: ~/.creamy/creamy-project)",
    envvar="CREAMY_PROJECT",
)
...
def _build_requirement(spec: str) -> str:
    if spec.startswith(("git@", "https://")):
        ...
        if has_ref:
            ref = f"@{ref}"
            return f"git+{CREAMY_CONTRIB_REPO}{ref}#subdirectory=packages/{name}"
...
def _ensure_project(project: Path) -> None:
    if (project / "pyproject.toml").is_file():
        return
    _uv("init", "--bare", "--name", "creamy-project", "--app", cwd=project)
    creamy_requirement = _build_creamy_requirement()
    _uv("add", "--active", "--no-sync", *creamy_requirement, cwd=project)
...
def install(
    ...
    """Install a plugin into Creamy's environment, or sync the environment if no specifications are provided."""
...
def update(
    ...
    """Update selected package or all packages in Creamy's environment."""
    _ensure_project(project)
    if not packages:
