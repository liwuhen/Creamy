...
def test_login_openai_runs_oauth_flow_and_prints_usage_hint(
    ...
    assert "CREAMY_MODEL=openai:gpt-5-codex" in result.stdout
...
def test_build_creamy_requirement_uses_direct_url_json(monkeypatch) -> None:
    ...
    assert cli._build_creamy_requirement() == ["git+https://github.com/bubbuild/bub.git@main#subdirectory=python"]


def test_build_creamy_requirement_falls_back_to_installed_version(monkeypatch) -> None:
    class FakeDistribution:
        version = "0.3.4"
        name = "creamy"
    ...
    assert cli._build_creamy_requirement() == ["creamy"]


def test_build_creamy_requirement_uses_local_path_for_file_dist(monkeypatch) -> None:
    class FakeDistribution:
        name = "creamy"
    ...
    assert cli._build_creamy_requirement() == ["/tmp/worktrees/bub"]  # noqa: S108


def test_build_creamy_requirement_marks_editable_local_dist(monkeypatch) -> None:
    class FakeDistribution:
        name = "creamy"
    ...
    assert cli._build_creamy_requirement() == ["--editable", "/tmp/worktrees/bub"]  # noqa: S108


def test_ensure_project_initializes_project_and_adds_creamy_dependency(tmp_path: Path, monkeypatch) -> None:
    project = tmp_path / "managed-project"
    project.mkdir()
    captured: list[tuple[tuple[str, ...], Path]] = []

    monkeypatch.setattr(cli, "_build_creamy_requirement", lambda: ["--editable", "/tmp/bub"])  # noqa: S108
    monkeypatch.setattr(cli, "_uv", lambda *args, cwd: captured.append((args, cwd)))

    cli._ensure_project(project)

    assert captured == [
        (("init", "--bare", "--name", "creamy-project", "--app"), project),
