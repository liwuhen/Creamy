...
def _build_impl(tmp_path: Path) -> tuple[CreamyFramework, BuiltinImpl]:
...
@pytest.mark.asyncio
async def test_build_prompt_with_non_image_media_adds_attachment_text_part(tmp_path: Path) -> None:
    _, impl = _build_impl(tmp_path)
    message = ChannelMessage(
        session_id="s",
        channel="tg",
    ...
    assert result[1] == {"type": "text", "text": "[Attached audio: audio/ogg]"}
