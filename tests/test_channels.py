...
def test_bub_message_filter_accepts_private_messages() -> None:
    ...
    assert CreamyMessageFilter().filter(message) is True
...
def test_bub_message_filter_requires_group_mention_or_reply() -> None:
    ...
    assert CreamyMessageFilter().filter(message) is False
...
def test_feishu_extract_reply_text_prefers_message_field() -> None:
    assert FeishuChannel._extract_reply_text('{"message":"hello"}') == "hello"
    assert FeishuChannel._extract_reply_text("plain") == "plain"
...
@pytest.mark.asyncio
async def test_feishu_handle_event_payload_builds_channel_message(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CREAMY_FEISHU_APP_ID", "cli_a97843a924b89cc0")
    monkeypatch.setenv("CREAMY_FEISHU_APP_SECRET", "wLVxog6hrOvtidWE8FDaRfVXYzX78jvO")
    ...
    channel = FeishuChannel(on_receive=on_receive)
...
@pytest.mark.asyncio
async def test_feishu_send_skips_blank_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CREAMY_FEISHU_APP_ID", "cli_a97843a924b89cc0")
    monkeypatch.setenv("CREAMY_FEISHU_APP_SECRET", "wLVxog6hrOvtidWE8FDaRfVXYzX78jvO")
    channel = FeishuChannel(lambda message: None)  # type: ignore[arg-type]
    sent: list[tuple[str, str]] = []
    ...
    await channel.send(_message('{"message":"hello"}', channel="feishu", chat_id="oc_1"))
    await channel.send(_message("   ", channel="feishu", chat_id="oc_1"))

    assert sent == [("oc_1", "hello")]
